...
class CV {
public:
    CV(Mutex& m) : _m(m) { pthread_cond_init (&_cv, NULL); }
    ~CV() { pthread_cond_destroy (&_cv); }

    void Wait() { pthread_cond_wait (&_cv, &_m._mutex); }
    void Signal() { pthread_cond_signal (&_cv); }
    void Broadcast() { pthread_cond_broadcast (&_cv); }
private:
    pthread_cond_t _cv;
    Mutex& _m;
};
...
public:
    ...
    void Run() { pthread_create (&_thread, &_tattr, _func, this); }

    int getIndex() const { return _index; }
    int getArg() const { return _arg; }
...
main (int argc, const char** argv)
{
	...
	printf ("Thread %d - (%f : %f) %f sec - %f nsec/pair\n", i,
		_timers[i].getStart(), _timers[i].getStop(), 
		dt, dt/(N/nthrd)*1e9);
    }
    printf ("Proc (wait) time = (%f : %f) %f sec\n", 
	    _timers[0].getStart(), _timers[0].getStop(),
	    _timers[0].getDiff());
